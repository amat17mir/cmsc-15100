...
;; returns a number of the menus with the 
(define (num-menus-with-cheaper-desserts menu)
  (count-if (filter (lambda ([n : MenuItem]) (= 'vegan (MenuItem-veg n))) menu))
             filter (lambda ([n : MenuItem])
                      (symbol=? 'dessert (MenuItem-kind n))) menu)
